...
class BucketTree:
    ...
    def insert(self, idn, value):  # 前q个区块插入
        ...
        # combined = sorted(zip(leaf_node.value, leaf_node.id, leaf_node.value_hash))
        #
        # value_s, id_s, hash_s = zip(*combined)
        # leaf_node.value = list(value_s)
        # leaf_node.id = list(id_s)
        # leaf_node.value_hash = list(hash_s)
        # for data in leaf_node.value:
        # leaf_node.segmentation = self.segmentation
        n = leaf_node
        while n is not None:
            n.compute_hash()
            n = n.parent_node
    ...
    def update(self, new_id, new_value):  # q+1个区块开始更新
        ...
        # 删除list1中的元素
        # del old_leaf_node.id[index_to_delete]
        old_leaf_node.id.pop(index_to_delete)
        # 删除list2中相应索引的元素
        # del old_leaf_node.value[index_to_delete]
        old_leaf_node.value.pop(index_to_delete)
        # del old_leaf_node.value_hash[index_to_delete]
        ...
        # content = str(new_id) + str(new_value)
        hash_of_data = hashlib.sha256(new_id.encode('utf-8')).hexdigest()
        # hash_of_data = hashlib.sha256(str(new_value).encode('utf-8')).hexdigest()
    ...
    def _range_query(self, node, id_set, query_start, query_end):
        ...
        if node.is_leaf_node():
            if node.range_end < query_start or node.range_start > query_end:
                ...
                # Leaf node
                # self.vo_1.append(node.r)
                # num = len(node.hash_value) // self.segmentation
                ...
                #
                #     if re in id_set:
                #         self.result.append(node.id[index])
                ...
                #     for value in range(query_start, query_end + 1):
                #         if value in j:
                #             flag[i] = 1
                #             break
                #         else:
                ...
                #         c = ''
                #         for j in i:
                #             c += hashlib.sha256(str(j).encode('utf-8')).hexdigest()
                #         add_array.append(hashlib.sha256(c.encode('utf-8')).hexdigest())
                #     else:
                #         for item in i:
                #             k = node.value.index(item)
    ...
    def _vo_construct(self, node, id_set, query_start, query_end):
        ...
        if node.is_leaf_node():
            if node.range_end < query_start or node.range_start > query_end:
                ...
                # print(len(node.value_hash) ,self.segmentation,num)
    ...
    def point_query(self, query_value):

        return self._point_query(self.root_node, query_value)

    point_result = []
    point_vo_1 = []

    def _point_query(self, node, query_value):

        if node.is_leaf_node():
            if node.range_end < query_value or node.range_start > query_value:
                # Pruned node
                self.point_vo_1.append(node.r)
                self.point_vo_1.append(node.hash_value)

            else:
                # Leaf node
                self.point_vo_1.append(node.r)

                indices = self.binary_search_all(node.value, query_value)
                while indices:
                    index = indices.pop(0)
                    self.point_result.append(node.id[index])
                ...
                for m, n in enumerate(flag):
                    ...
                    if n == 0:
                        ...
                        for item in i:
                            k = node.value.index(item)
                            if node.id[k] in self.result:
                                a.append((node.id[k], node.value[k]))
                            else:
                                a.append(node.value_hash[k])

                        add_array.append(a)
                self.point_vo_1.append(add_array)
                ...
                #     i = grouped_data[m]
                #     if n == 0:
                #         add_array.append(hashlib.sha256(str(i).encode('utf-8')).hexdigest())
                #     else:
                #         add_array.append(i)
                # self.point_vo_1.append(add_array)
            ...
            if node.range_end < query_value or node.range_start > query_value:
                ...
                # Non-leaf node
                self.point_vo_1.append('[')
                self._point_query(node.left_child, query_value)
                self._point_query(node.right_child, query_value)
                self.point_vo_1.append(']')
        return self.point_vo_1

# tree = BucketTree(0, 576, 4)
# tree.insert(0, 21)
# tree.insert(1, 169)
# tree.insert(2, 289)
# tree.insert(3, 363)
...
# tree.insert(8, 19)
#
# t = tree.tree_node_content()
# print(t)
# vo1 = tree.range_query(165, 356)
# print(vo1)
# print(tree.result)
# new = tree.hash_root(vo1)
# print(tree.root_node.hash_value)
# print(new)
# print(new == tree.root_node.hash_value)
